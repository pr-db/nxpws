...
class NXPTrackVision(Node):
    ...
    def __init__(self):
        ...
        self.maskRectRatioWidthHeight = np.array(
            [float(self.mask_ratio_array[0]), float(self.mask_ratio_array[1])])
        ...
        self.pixyImageWidth = 100
        self.pixyImageHeight = 68
    ...
    def findLines(self, passedImage):
        ...
        #################EDITED CODE################
        """
        1)Converting BGR image to HSV to detect only white (255,255,255) BGR value
        2)Mask Out the selected Region 
        3)Detect the Contours form Canny Image and mark Out them with Seperate colour 
        Here seperate colour BGR(0, 255, 204) & HSV (36, 255, 255)
        """
        imghsv = cv2.cvtColor(passedImage, cv2.COLOR_BGR2HSV)
        lower = np.array([0, 0, 250])
        upper = np.array([0, 0, 255])
        mask = cv2.inRange(imghsv, lower, upper)
        # Blur the image for Reduction of Noise
        mask = cv2.GaussianBlur(mask, (5, 5), 3)
        mask = cv2.Canny(mask, 200, 400)  # Detect Edges of the White portion
        # create a white mask to cover a selected region
        maskWhite = np.ones(mask.shape[:2], dtype="uint8") * 255
        mask_ratio_array = [1.0, 0.4]
        maskRectRatioWidthHeight = np.array(
            [float(mask_ratio_array[0]), float(mask_ratio_array[1])])
        imageHeight, imageWidth = mask.shape[:2]
        ...
        passedImageGrayThreshMasked = cv2.bitwise_and(
            mask, mask, mask=maskVehicle)

        ###Detect Contours and find their area accordingly###
        cnts, hierarchy = cv2.findContours(passedImageGrayThreshMasked.copy(),
                                           cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        for cn in cnts:
            area = cv2.contourArea(cn)
            if area <= 590:
                # draw contours with unique color##
                cv2.drawContours(passedImage, cn, -1, (0, 255, 204), 3)

        ################end###########

        # Image dimensions
        ### Create HSV for unique color ##
        imageHeight, imageWidth = passedImage.shape[:2]
        passedImageGrayThresh = cv2.cvtColor(passedImage, cv2.COLOR_BGR2HSV)
        lower = np.array([36, 255, 255])
        upper = np.array([36, 255, 255])
        passedImageGrayThresh = cv2.inRange(
            passedImageGrayThresh, lower, upper)        # Create image mask background
        maskWhite = np.ones(
            passedImageGrayThresh.shape[:2], dtype="uint8") * 255
        ...
        for cn in cnt:
            ...
            [vectorX, vectorY, linePointX, linePointY] = cv2.fitLine(
                cn, cv2.DIST_L2, 0, 0.01, 0.01)
            ...
            if (linePointX >= 1.0) and (linePointY >= 1.0):
                ...
                bottomRightX = int(
                    ((imageHeight-linePointY)*vectorX/vectorY)+linePointX)
                topLeftY = 0
                ...
                if (topLeftX <= 0) and (bottomRightX > imageWidth):
                    ...
                    bottomRightY = int(
                        ((imageWidth-linePointX)*vectorY/vectorX)+linePointY)
                    topLeftX = 0
                    ...
                    topLeftY = int(
                        ((imageWidth-linePointX)*vectorY/vectorX)+linePointY)
                    bottomRightY = int(
                        ((-linePointX)*vectorY/vectorX)+linePointY)
                    topLeftX = imageWidth
                    bottomRightX = 0
                    ...
                    bottomRightY = int(
                        ((imageWidth-linePointX)*vectorY/vectorX)+linePointY)
                    topLeftX = int(((imageHeight-linePointY) *
                                   vectorX/vectorY)+linePointX)
                    bottomRightX = imageWidth
                    ...
                    topLeftX = 0
                    bottomRightX = int(
                        (linePointY)*(-vectorX/vectorY)+linePointX)
                ...
                bottomRightXScaled = int(
                    bottomRightX*(self.pixyImageWidth/imageWidth))
                topLeftYScaled = int(topLeftY*(self.pixyImageWidth/imageWidth))
                bottomRightYScaled = int(
                    bottomRightY*(self.pixyImageWidth/imageWidth))
